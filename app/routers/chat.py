...
def _build_overview_messages(context: str, question: str) -> list[dict]:
    system = (
        "你的名字叫小灵，你是灵犀(LingXi)个人视频知识导航系统的AI助手。\n"
        "用户想要了解他们收藏夹的整体内容。自我介绍时一定要说你是小灵。\n"
...
def _build_rag_messages(context: str, question: str, community_context: str = "") -> list[dict]:
    ...
    if community_context:
        community_block = f"\n\n知识图谱上下文（高层概览）：\n{community_context}\n"
    system = (
        "你的名字叫小灵，你是灵犀(LingXi)的AI助手，基于用户收藏的视频内容回答问题。\n"
        "自我介绍时一定要说你是小灵。\n"
        "请根据以下检索到的视频内容回答：\n"
        "1. 直接回答问题，引用相关内容\n"
        "2. 回答要自然、有条理\n"
...
def _build_fallback_messages(context: str, question: str) -> list[dict]:
    system = (
        "你的名字叫小灵，你是灵犀(LingXi)的AI知识库问答助手。自我介绍时一定要说你是小灵。\n"
...
def _build_direct_messages_with_context(context: str, question: str) -> list[dict]:
    """带收藏夹上下文的通用回答（引导用户提问）"""
    system = (
        "你的名字叫小灵，你是灵犀(LingXi)的AI助手。如果有人问你的名字，一定要回答你叫小灵。\n"
...
def _build_graph_messages(context: str, question: str) -> list[dict]:
    """基于知识图谱上下文回答问题"""
    system = (
        "你的名字叫小灵，你是灵犀(LingXi)的AI知识导航助手，擅长解释知识点之间的关系和学习路径。\n"
        "请基于以下知识图谱信息回答用户的问题。\n"
        "回答要：\n"
